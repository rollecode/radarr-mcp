...
from radarr_mcp import runtime
...
def test_missing_api_key_says_where_to_find_it(monkeypatch):
    ...
    assert "RADARR_API_KEY" in result["message"]
...
def test_every_tool_registers():
    ...
    from radarr_mcp import tools  # noqa: F401 -- registers the tools

    registered = asyncio.run(runtime.mcp.list_tools())
    assert len(registered) == 237
