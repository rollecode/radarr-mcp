...
mcp = FastMCP(
    ...
    instructions=(
        "Manage a Radarr instance: movies, movie files, the download queue, "
        "indexers, quality and custom formats, import lists, notifications, "
        "tags and system settings. Every API operation is a tool, named "
        "verb-first: list_* reads a collection, get_*_by_id reads one record, "
        "create_*, update_* and delete_* change them. "
        "Before writing a resource, read the matching list_* or its "
        "list_*_schema to see the fields it expects, then send the whole "
        "object back as body -- Radarr replaces the record rather than "
        "merging. Use list_movie_lookup to find a movie before adding it, and "
